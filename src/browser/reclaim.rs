...
pub fn decide(tab: TabState, settings: &Settings) -> ReclaimAction {
    // The active tab must always be fully live.
    if tab.is_active {
...
mod tests {
    ...
    fn the_active_tab_is_never_reclaimed() {
        ...
        assert_eq!(decide(state, &aggressive()), ReclaimAction::None);
    }
    ...
    fn activating_a_sleeping_tab_restores_it() {
        for power in [TabPower::Suspended, TabPower::Discarded, TabPower::LowMemory] {
            ...
            assert_eq!(decide(state, &balanced()), ReclaimAction::Restore, "{power:?}");
        }
    }

    #[test]
    fn a_fresh_background_tab_only_lowers_its_memory_target() {
        assert_eq!(decide(tab(5, TabPower::Normal), &balanced()), ReclaimAction::LowerMemoryTarget);
        // ...and is not asked twice.
        assert_eq!(decide(tab(5, TabPower::LowMemory), &balanced()), ReclaimAction::None);
    }

    #[test]
    fn passing_the_suspend_threshold_suspends() {
        assert_eq!(decide(tab(119, TabPower::LowMemory), &balanced()), ReclaimAction::None);
        assert_eq!(decide(tab(120, TabPower::LowMemory), &balanced()), ReclaimAction::Suspend);
        assert_eq!(decide(tab(500, TabPower::Suspended), &balanced()), ReclaimAction::None);
    }

    #[test]
    fn balanced_mode_never_discards() {
        assert_eq!(decide(tab(86_400, TabPower::Suspended), &balanced()), ReclaimAction::None);
    }

    #[test]
    fn aggressive_mode_discards_long_idle_tabs() {
        assert_eq!(decide(tab(30, TabPower::Normal), &aggressive()), ReclaimAction::Suspend);
        assert_eq!(decide(tab(600, TabPower::Suspended), &aggressive()), ReclaimAction::Discard);
        // Once discarded there is nothing left to reclaim.
        assert_eq!(decide(tab(9_999, TabPower::Discarded), &aggressive()), ReclaimAction::None);
    }

    #[test]
    fn audible_tabs_are_never_frozen() {
        let playing = TabState { is_audible: true, ..tab(10_000, TabPower::Normal) };
        assert_eq!(decide(playing, &aggressive()), ReclaimAction::LowerMemoryTarget);

        let already_trimmed = TabState { is_audible: true, ..tab(10_000, TabPower::LowMemory) };
        assert_eq!(decide(already_trimmed, &aggressive()), ReclaimAction::None);
    }

    #[test]
    fn a_tab_that_starts_playing_while_suspended_is_restored() {
        let state = TabState { is_audible: true, ..tab(10_000, TabPower::Suspended) };
        assert_eq!(decide(state, &aggressive()), ReclaimAction::Restore);
    }

    #[test]
    fn turning_reclaiming_off_wakes_sleeping_tabs() {
        let off = Settings { reclaim_mode: ReclaimMode::Off, ..Default::default() };
        assert_eq!(decide(tab(10_000, TabPower::Suspended), &off), ReclaimAction::Restore);
        assert_eq!(decide(tab(10_000, TabPower::Normal), &off), ReclaimAction::None);
    }
